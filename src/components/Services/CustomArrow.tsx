...
export const CustomArrow = ({ clickHandler, hasArrow, label }: CustomArrowProps) => {
  ...
  return (
    <button type='button' onClick={clickHandler} className={buttonStyle} aria-label={label} disabled={!hasArrow}>
      {isNext ? <GrFormNext size={22} className='mb-1 ml-2' /> : <GrFormPrevious size={22} />}
    </button>
  );
};
