...
export const CommentList: React.FC<CommentListProps> = ({ postId }) => {
  ...
  const { comments, errorMessage, getComments, likeComment } =
    useCommentStore();
  ...
  const handleLikeClick = async (commentId: string) => {
    if (!user)
      return toastNotification(
        'error',
        'Debes iniciar sesión para poder dar like a un comentario'
      );
    ...
    <div className='space-y-4'>
      {recentComments.map((comment) => (
        <div
          ...
          className='p-2 font-sans rounded-md bg-light-gray'
        >
          <div className='flex items-center w-full px-2 py-1 rounded-md bg-navy-blue'>
            <h4 className='text-base font-medium text-turquoise'>
              {comment.user.name}
            </h4>
            <small className='mx-2 mt-1 text-light-gray text-2xs'>
